...
package Coypu.Tests.When_interacting_with_the_browser
{
    ...
    public class When_clicking_links : BrowserInteractionTests
    {
        ...
        {
            assertThat(driver.ClickedElements, Has.Member(linkToBeClicked));
        }
        ...
        {
            assertThat(driver.FindLinkRequests.Contains("Some link locator"), "Wait called before find");
        }

        private void AssertButtonNotClickedYet(StubElement linkToBeClicked)
        {
            assertThat(driver.ClickedElements, Has.No.Member(linkToBeClicked), "Expected link not to have been clicked yet, but it has been");
        }
